...
static subreactor_state_t subreactor_state_pool[SUBREACTOR_THREAD_NUM];
...
int clientfd_event_register(int clientfd, int epollfd);
...
int listenfd_event_register(int listenfd, int epollfd);
conn_state_t* create_connstate(int fd, int epollfd, RCALLBACK r_callback, WCALLBACK w_callback);

int fd_queue_init(fd_queue_t* fd_queue);
int fd_queue_destroy(fd_queue_t* fd_queue);

int submit_clientfd_to_subreactor(int clientfd, int index);
int handle_pending_fds(int index);
...
int set_event(conn_state_t* conn_state, int event,int flag){
    ...
    if(flag == FLAG_ADD){
      ...
      if(epoll_ctl(conn_state->epollfd, EPOLL_CTL_ADD, conn_state->fd, &ev) < 0){
             return -1;
        }
        ...
        if(conn_state->current_events == 0){
            ...
            if(epoll_ctl(conn_state->epollfd, EPOLL_CTL_MOD, conn_state->fd, &ev) < 0){
                return -1;
            }
        }
        conn_state->current_events = event;
        ...
        if(epoll_ctl(conn_state->epollfd, EPOLL_CTL_DEL, conn_state->fd, &ev) < 0){
            return -1;
        }
...
int clientfd_event_register(int clientfd, int epollfd){
    ...
    conn_state_t* conn_state = create_connstate(clientfd, epollfd, recv_callback, send_callback);
...
void* subreactor_callback(void* arg){
    ...
    int index = *(int*)(arg);
    subreactor_state_t* sub_state = &subreactor_state_pool[index];

    int epollfd = epoll_create(1);
    struct epoll_event events[EPOLL_EVENT_SIZE] = {0};
    sub_state->epollfd = epollfd;
    
    conn_state_t* state = create_connstate(sub_state->fdqueue_eventfd, sub_state->epollfd, NULL, NULL);
    set_event(state, EPOLLIN | EPOLLET, FLAG_ADD);
    ...
    while(1){
        ...
        for(int i = 0; i < nready; ++i){
            ...
            if(events[i].events & EPOLLIN){
                if(conn_state->fd == sub_state->fdqueue_eventfd){
                    while(1){
                        uint64_t val;
                        int read_len = eventfd_read(conn_state->fd, &val); // 读取以清空计数器
                ...
                else conn_state->r_cb(conn_state);
            }
...
int submit_clientfd_to_subreactor(int clientfd, int index)
{
    subreactor_state_t *subreactor = &subreactor_state_pool[index];

    pthread_mutex_lock(&subreactor->fd_queue.lock);

    if (subreactor->fd_queue.count == subreactor->fd_queue.capacity) // 不太容易走到此分支，所以占用锁的时间还是很短的
    {
...
int handle_pending_fds(int index){
    subreactor_state_t *state = &subreactor_state_pool[index];
    ...
    for (int i = 0; i < count; i++){
        ...
        clientfd_event_register(fd, state->epollfd);
    }
...
int listenfd_event_register(int listenfd, int epollfd){ //
    ...
    conn_state_t* conn_state = create_connstate(listenfd, epollfd, accept_callback, NULL);
    if(conn_state == NULL){
        return -1;
    }
...
int accept_callback(conn_state_t* conn_state){
    ...
    while(1){
        ...
        if(submit_clientfd_to_subreactor(clientfd, clientfd % SUBREACTOR_THREAD_NUM) < 0){
            printf("submit_clientfd_to_subreactor failed.\n");
            close(clientfd);
            continue;
        }
    }
...
int fd_queue_destroy(fd_queue_t *fd_queue){
    if(fd_queue == NULL) return 0;
    if(fd_queue->clientfds) free(fd_queue->clientfds);
    pthread_mutex_destroy(&fd_queue->lock);
    fd_queue->clientfds = NULL;
    fd_queue->capacity = 0;
    fd_queue->count = 0;
    fd_queue->head = fd_queue->tail = 0;
    return 0;
}
...
int main(int argc, char** argv){
    ...
    int main_epollfd = epoll_create(1);
    struct epoll_event events[EPOLL_EVENT_SIZE] = {0};
    ...
    if(listenfd_event_register(listenfd, main_epollfd) < 0){
        printf("listenfd_event_register failed.\n");
        return -1;
    }

    for (int i = 0; i < SUBREACTOR_THREAD_NUM; ++i){
        subreactor_state_pool[i].index = i;
        ...
        subreactor_state_pool[i].fdqueue_eventfd = fdqueue_eventfd;
        if(fd_queue_init(&subreactor_state_pool[i].fd_queue) < 0){
            printf("fd_queue_init failed.\n");
            return -2;
        }
        pthread_create(&subreactor_state_pool[i].threadid, NULL, subreactor_callback, &subreactor_state_pool[i].index);
    }

    while(1){
        int nready = epoll_wait(main_epollfd, events, EPOLL_EVENT_SIZE, -1);
        if(nready < 0){
            perror("epoll_wait");
            continue;
        }
